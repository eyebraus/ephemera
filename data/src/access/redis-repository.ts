...
> {
    getDocId: (id: TId) => string;
    getId: (docId: string) => TId;
...
export interface RedisRepository<TDocument extends RedisDocument, TId extends NonNullable<unknown> = string> {
     ...
     * @returns A copy of the provided document with its docId populated.
     ...
     */
    save: (id: TId, doc: Omit<TDocument, 'docId'>) => Promise<TDocument>;
...
const RedisRepository = <TDocument extends RedisDocument, TId extends NonNullable<unknown> = string>(
    ...
    const { getDocId, getId, mapEntityToDocument } = configuration;
    ...
    const redisRepository: RedisRepository<TDocument, TId> = {
        ...
        expire: async (id, ttlInSeconds) => {
            ...
            await repository.expire(docId, ttlInSeconds);
        },

        expireAt: async (id, expirationDate) => {
            const docId = getDocId(id);
            await repository.expireAt(docId, expirationDate);
        },

        fetch: async (id) => {
            const docId = getDocId(id);
            const entity = await repository.fetch(docId);
        ...
        remove: async (id) => {
            const docId = getDocId(id);
            await repository.remove(docId);
        },

        save: async (id, doc) => {
            const docId = getDocId(id);
            const entity = await repository.save(docId, { ...doc, docId });
            return mapEntityToDocument(entity);
        },
...
const SearchBuilder = <TDocument extends RedisDocument, TId extends NonNullable<unknown>>(
    ...
    allIds: async () => {
        ...
        return entities.map((entity) => {
            const { docId } = mapEntityToDocument(entity);
            return getId(docId);
        });
    },
...
export const redisRepositoryBuilder: Factory<DataProfile, RedisRepositoryBuilder> = (getUnit) => {
    ...
    return (configuration) => {
        const { getDocId, getId, mapEntityToDocument, schema } = configuration;
        const repository = new Repository(schema, redisClient);

        return RedisRepository(repository, { getDocId, getId, mapEntityToDocument });
    };
};
